...
func TestRekeyContactPoints_FromPlaintext(t *testing.T) {
	...
	m := migration.NewMigrator(noop.NoOpEncoder{}, newEnc, cfg, stor, resources.NewHelpers())
	report, err := m.Rekey(migration.RekeyOptions{NoBackup: true})
	is.NoErr(err)
	is.Equal(len(report.Errors), 0)
	is.Equal(len(report.ContactPointsFiles), 1)
...
func TestRekeyContactPoints_ToPlaintext(t *testing.T) {
	...
	m := migration.NewMigrator(oldEnc, noop.NoOpEncoder{}, cfg, stor, resources.NewHelpers())
	report, err := m.Rekey(migration.RekeyOptions{NoBackup: true})
	is.NoErr(err)
	is.Equal(len(report.Errors), 0)
...
func TestRekeyContactPoints_FileNotFound_IsSkipped(t *testing.T) {
	...
	m := migration.NewMigrator(prefixEncoder{"OLD:"}, prefixEncoder{"NEW:"}, cfg, stor, resources.NewHelpers())
...
func TestRekeySecureDataFiles_UnsupportedExtension(t *testing.T) {
	...
	m := migration.NewMigrator(noop.NoOpEncoder{}, prefixEncoder{"ENC:"}, cfg, stor, resources.NewHelpers())
	report, err := m.Rekey(migration.RekeyOptions{NoBackup: true})
	is.NoErr(err)
...
func TestRekey_DryRun_ContactPoints_PopulatesPreviews(t *testing.T) {
	...
	m := migration.NewMigrator(oldEnc, prefixEncoder{"NEW:"}, cfg, stor, resources.NewHelpers())
