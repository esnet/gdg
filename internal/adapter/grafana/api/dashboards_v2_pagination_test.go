...
// pagedDashboardServer fakes the App Platform list contract, serving `total`
// dashboards across pages of `pageSize` via metadata.continue (empty on the
// last page). It also stubs /api/org and /api/search so the full
// listDashboardsV2 path runs without a real Grafana, and records every
// continue token so the test can confirm all pages were walked, not skipped
// or looped forever.
