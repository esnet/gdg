...
func (s *DashNGoImpl) ListDashboardPermissions(filterReq filters.Filter) ([]DashboardAndPermissions, error) {
	validateDashboardEnterpriseSupport(s.grafanaConf)
...
func (s *DashNGoImpl) DownloadDashboardPermissions(filterReq filters.Filter) ([]string, error) {
	...
	for _, link := range boardLinks {
		if link.Permissions == nil || len(link.Permissions) == 0 {
			continue
		}
...
func validateDashboardEnterpriseSupport(grafanaConf *config.GrafanaConfig) {
	if !grafanaConf.IsEnterprise() {
		slog.Error("Enterprise support is required for Dashboard Permissions")
		os.Exit(1)
	}
}
