...
func (s *DashNGoImpl) DownloadAlertTemplates() (string, error) {
	...
	dsPath := buildResourcePath(templatesFile, config.AlertingResource, s.isLocal(), false)
...
func (s *DashNGoImpl) UploadAlertTemplates() ([]string, error) {
	...
	fileLocation := buildResourcePath(templatesFile, config.AlertingResource, s.isLocal(), false)
