...
type GrafanaConfigOption func(*GrafanaConfig)

func WithSecureAuth(auth SecureModel) GrafanaConfigOption {
	return func(s *GrafanaConfig) {
		s.secureAuth = &auth
	}
}

func WithContextName(contextName string) GrafanaConfigOption {
	return func(s *GrafanaConfig) {
		s.contextName = contextName
	}
}

func NewGrafanaConfig(opts ...GrafanaConfigOption) *GrafanaConfig {
	cfg := &GrafanaConfig{}
...
func (s *GrafanaConfig) Apply(opts ...GrafanaConfigOption) *GrafanaConfig {
	for _, opt := range opts {
		opt(s)
	}
	return s
